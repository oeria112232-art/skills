...
import {
  ...
  Home, UserCircle, Settings, Wallet, ShieldCheck, Video, Sparkles
} from "lucide-react";
...
const adminLinks = [
  ...
  { href: "/admin/security-monitor", icon: ShieldCheck, label: "Security Control", arLabel: "غرفة التحكم والأمان" },
  ...
  { href: "/admin/deposits", icon: Wallet, label: "Subscriptions & Deposits", arLabel: "الاشتراكات وشحن النقاط" },
  { href: "/admin/points-rules", icon: Sparkles, label: "Earned Points Rules", arLabel: "قواعد النقاط المكتسبة" },
  { href: "/user/settings", icon: UserCircle, label: "My Account", arLabel: "حسابي" },
];
      ...
      <div className="flex items-center justify-between gap-2.5 px-4 py-5 border-b border-sidebar-border/50">
        ...
        <div className="hidden lg:flex items-center gap-2.5 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <span className="font-bold text-lg leading-tight truncate text-foreground tracking-tight">{isAr ? "مهارات" : "Mharat"}</span>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold bg-primary/10 text-primary dark:text-emerald-400 capitalize">
                {isAr
                  ? (user.role === "super_admin" ? "أدمن رئيسي" : user.role === "admin" ? "مسؤول" : user.role === "instructor" ? "معلم" : user.role === "company" ? "شركة" : "طالب")
                  : user.role}
              </span>
            </div>
          </div>
        ...
        <>
          ...
          <div className="fixed top-0 left-0 right-0 h-16 border-b bg-background/90 backdrop-blur-md z-40 flex items-center justify-between px-4 shadow-sm">
            <div className={cn("flex items-center gap-3", isAr ? "order-2" : "order-1")}>
              <SkillsLogo className="h-8 w-8" />
              <span className="font-bold text-lg text-foreground tracking-tight">{isAr ? "مهارات" : "Mharat"}</span>
            </div>
