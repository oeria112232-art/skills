...
export default function AdminDashboard() {
  ...
  const statCards = stats ? [
    ...
    { label: "Open Jobs", value: stats.openJobs, sub: `of ${stats.totalJobs} total`, icon: Briefcase, color: "text-green-500", link: "/admin/jobs" },
    { label: "Pending Applications", value: stats.pendingApplications, sub: `of ${stats.totalApplications} total`, icon: TrendingUp, color: "text-orange-500", link: "/admin/applications" },
    { label: "Upcoming Workshops", value: stats.upcomingWorkshops, sub: `of ${stats.totalWorkshops} total`, icon: BookOpen, color: "text-violet-500", link: "/admin/workshops" },
    { label: "Certificates Issued", value: stats.totalCertificates, icon: Award, color: "text-yellow-500", link: "/certificates" },
  ] : [];

  return (
    <AppLayout>
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-1" data-testid="heading-admin-dashboard">Admin Dashboard</h1>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          ...
          <Link href="/admin/applications" className="text-xs text-primary hover:underline" data-testid="link-view-all-applications">View all</Link>
        </div>
        {isLoading ? (
              ...
              <div key={app.id} className="px-5 py-3 flex items-center justify-between gap-4" data-testid={`recent-app-${app.id}`}>
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  {app.screeningScore != null && (
                    <p className="text-xs text-muted-foreground mt-1">Score: {app.screeningScore}%</p>
                  )}
                </div>
              </div>
            ))}
