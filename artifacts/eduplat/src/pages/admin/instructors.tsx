...
const ADMIN_PAGES = [
  ...
  { id: "consultations", label: "Consultations", arLabel: "الاستشارات" },
];

const CONSULTATION_SPECIALIZATIONS = [
  { id: "tot", label: "إعداد المدربين TOT" },
  { id: "networking", label: "شبكات CCNA" },
  { id: "cybersecurity", label: "الأمن السيبراني" },
  { id: "fullstack", label: "تطوير الويب الشامل" },
  { id: "computer-basics", label: "أساسيات الحاسوب" },
  { id: "mobile", label: "تطوير تطبيقات الموبايل" },
  { id: "other", label: "استشارات عامة" },
];
...
export default function AdminInstructorsPage() {
  ...
  const instructors = usersList.filter((u: any) => u.role === "instructor" || u.role === "admin" || u.role === "super_admin");
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(instructor.allowedPages || []).length === 0 ? (
                    ...
                    instructor.allowedPages?.map((p: string) => {
                      if (p.startsWith("consultation_cat:")) {
                        const catId = p.replace("consultation_cat:", "");
                        const spec = CONSULTATION_SPECIALIZATIONS.find(s => s.id === catId);
                        return (
                          <Badge key={p} variant="outline" className="text-[10px] font-semibold border-amber-500/40 text-amber-600 dark:text-amber-400 bg-amber-500/10">
                            {isAr ? `تخصص: ${spec ? spec.label : catId}` : `Spec: ${spec ? spec.label : catId}`}
                          </Badge>
                        );
                      }
              ...
              <Dialog open={isDialogOpen && selectedInstructorId === instructor.id} onOpenChange={(open) => !open && setIsDialogOpen(false)}>
                ...
                <DialogContent className="max-h-[85vh] overflow-y-auto">
                  ...
                  <div className="space-y-4 py-4">
                    ...
                    {/* Consultation Categories Specialization Block */}
                    {tempAllowedPages.includes("consultations") && (
                      <div className="pt-4 border-t space-y-3">
                        <div className="space-y-1">
                          <h4 className="text-xs font-extrabold text-amber-500 uppercase tracking-wider">
                            {isAr ? "تحديد اختصاص الاستشارات للمعلم" : "Instructor Consultation Specializations"}
                          </h4>
                          <p className="text-xs text-muted-foreground">
                            {isAr 
                              ? "اختر التخصصات التي سيُسمح للمعلم بفرز واستقبال استشاراتها فقط:" 
                              : "Select the consultation categories assigned to this instructor:"}
                          </p>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {CONSULTATION_SPECIALIZATIONS.map(spec => {
                            const catKey = `consultation_cat:${spec.id}`;
                            const isCatSelected = tempAllowedPages.includes(catKey);
                            return (
                              <div
                                key={spec.id}
                                onClick={() => togglePage(catKey)}
                                className={`flex items-center justify-between p-2.5 rounded-lg border cursor-pointer transition-all ${
                                  isCatSelected ? "border-amber-500 bg-amber-500/10 text-amber-400" : "border-border hover:bg-muted/40"
                                }`}
                              >
                                <span className="text-xs font-bold">{spec.label}</span>
                                <div className={`w-4 h-4 rounded border flex items-center justify-center ${
                                  isCatSelected ? "bg-amber-500 border-amber-500 text-slate-950" : "border-muted-foreground"
                                }`}>
