...
router.get("/consultations", requireAuth, async (req: AuthenticatedRequest, res): Promise<void> => {
  try {
    ...
    if (req.user?.role === "admin") {
      ...
      // Instructors see consultations assigned to them OR matching their specified category specializations
      const allowedPages: string[] = (req.user.allowedPages as string[]) || [];
      const catFilters = allowedPages
        .filter((p: string) => p.startsWith("consultation_cat:"))
        .map((p: string) => p.replace("consultation_cat:", ""));

      const all = await db.select().from(consultationsTable).orderBy(desc(consultationsTable.createdAt));
      consultations = all.filter((c) => {
        // Always include consultations directly assigned to this instructor
        if (c.assignedTo === req.user!.id) return true;
        // If specific category specializations are set by admin, match against category
        if (catFilters.length > 0) {
          return catFilters.includes(c.category);
        }
        // Fallback: if no specific category restriction set, show all
        return true;
      });
    } else {
...
router.post("/consultations", requireAuth, consultationRateLimit, async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  if (assignedTo) {
    ...
    if (!assignedUser || (assignedUser.role !== "admin" && assignedUser.role !== "instructor" && assignedUser.role !== "super_admin")) {
      res.status(400).json({ error: "Consultation can only be assigned to a valid admin, super_admin or instructor" });
      return;
    }
  }
