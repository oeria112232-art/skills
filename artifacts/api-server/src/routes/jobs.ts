...
router.patch("/jobs/:id", requireAuth, requireRole(["admin", "company"]), async (req: AuthenticatedRequest, res): Promise<void> => {
  const params = UpdateJobParams.safeParse(req.params);
  if (!params.success) { res.status(400).json({ error: "Invalid id" }); return; }

  const [existingJob] = await db.select().from(jobsTable).where(eq(jobsTable.id, params.data.id));
  if (!existingJob) { res.status(404).json({ error: "Job not found" }); return; }

  const user = req.user!;
  if (user.role === "company" && existingJob.companyId !== user.id) {
