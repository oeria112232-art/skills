...
router.post("/workshops/:id/exam/submit", requireAuth, async (req: AuthenticatedRequest, res): Promise<void> => {
  ...
  // Verify enrollment
  const [enrollment] = await db.select().from(enrollmentsTable).where(and(eq(enrollmentsTable.workshopId, workshop.id), eq(enrollmentsTable.userId, req.user!.id)));
  if (!enrollment && req.user?.role !== "admin") {
    res.status(403).json({ error: "You must be enrolled in this workshop to submit the exam" });
    return;
  }
