...
const DEFAULT_SETTINGS: Record<string, { value: string; label: string }> = {
  ...
  earned_points_active_flag: { value: "true", label: "تفعيل نظام نقاط الإنجاز التعليمي" },
  earned_points_workshop_completion: { value: "50", label: "نقاط إتمام ورشة العمل" },
  earned_points_track_completion: { value: "200", label: "نقاط إتمام المسار التعليمي" },
  earned_points_quiz_high_score: { value: "25", label: "مكافأة اجتياز الاختبار بنسبة 90%+" },
  earned_points_attendance_bonus: { value: "15", label: "مكافأة الحضور التفاعلي المباشر" },
  earned_points_daily_streak: { value: "5", label: "مكافأة الدخول اليومي المستمر (Streak)" },
  earned_points_first_enrollment: { value: "10", label: "مكافأة التسجيل في أول مسار" },
...
router.get("/points/platform-info", requireAuth, async (_req: AuthenticatedRequest, res): Promise<void> => {
  const cacheKey = "cache:points:platform-info";
  try {
  ...
  try {
    ...
    try {
      await redis.set(cacheKey, JSON.stringify(merged), "EX", 1800); // 30 minutes TTL
    } catch (err) {
...
router.post("/admin/platform-settings", requireAuth, requireRole(["admin"]), async (req: AuthenticatedRequest, res): Promise<void> => {
  try {
    ...
    // Invalidate Redis cache on settings change
    try {
      await redis.del("cache:points:platform-info");
    } catch (e) {}

    await logAuditEvent({ action: "admin_update_platform_settings", userId: req.user!.id, targetType: "system", details: { count: settings.length }, req });
...
// A17. GET /points/earned-rules - Get current earned learning points rules
router.get("/points/earned-rules", async (_req, res): Promise<void> => {
  try {
    const dbSettings = await db.select().from(platformSettingsTable);
    const dbMap: Record<string, string> = {};
    for (const s of dbSettings as any[]) {
    ...
    const rules = {
      enabled: (dbMap["earned_points_active_flag"] ?? "true") === "true",
      workshopCompletion: parseInt(dbMap["earned_points_workshop_completion"] ?? "50", 10),
      trackCompletion: parseInt(dbMap["earned_points_track_completion"] ?? "200", 10),
      quizHighScoreBonus: parseInt(dbMap["earned_points_quiz_high_score"] ?? "25", 10),
      attendanceBonus: parseInt(dbMap["earned_points_attendance_bonus"] ?? "15", 10),
      dailyStreakBonus: parseInt(dbMap["earned_points_daily_streak"] ?? "5", 10),
      firstEnrollmentBonus: parseInt(dbMap["earned_points_first_enrollment"] ?? "10", 10),
    };
...
// A18. POST /points/award-earned - Secure endpoint to credit earned learning points
router.post("/points/award-earned", requireAuth, async (req: AuthenticatedRequest, res): Promise<void> => {
  try {
    const userId = req.user!.id;
    ...
    if ((dbMap["earned_points_active_flag"] ?? "true") !== "true") {
      res.status(400).json({ error: "Earned points reward system is currently disabled" });
      return;
    }
    ...
    switch (action) {
      case "workshop_completion":
        pointsToAward = parseInt(dbMap["earned_points_workshop_completion"] ?? "50", 10);
        description = "مكافأة إتمام ورشة عمل تعليمية";
        break;
      case "track_completion":
        pointsToAward = parseInt(dbMap["earned_points_track_completion"] ?? "200", 10);
        description = "مكافأة إتمام مسار تعليمي كامل";
        break;
      case "quiz_high_score":
        pointsToAward = parseInt(dbMap["earned_points_quiz_high_score"] ?? "25", 10);
        description = "مكافأة التفوق في الاختبار النهائي بنسبة 90%+";
        break;
      case "attendance_bonus":
        pointsToAward = parseInt(dbMap["earned_points_attendance_bonus"] ?? "15", 10);
        description = "مكافأة الحضور التفاعلي المباشر للورشة";
        break;
      case "daily_streak":
        pointsToAward = parseInt(dbMap["earned_points_daily_streak"] ?? "5", 10);
        description = "مكافأة الاستمرارية والدخول اليومي المستمر";
        break;
      default:
    ...
    if (pointsToAward <= 0) {
      res.json({ success: false, message: "No points configured for this action", awarded: 0 });
      return;
    }
    ...
    try {
      ...
      await verifyAndHardenUserBalance(user);
      const newPoints = (user.points || 0) + pointsToAward;

      await updateAndSignUserBalance(userId, newPoints);
      await insertSecureTransaction(
        null,
        userId,
        pointsToAward,
      ...
      await logAuditEvent({
        action: "earned_points_awarded",
        userId,
        targetId: userId,
        targetType: "user",
        details: { action, pointsToAward, newPoints, referenceId },
        req,
      });

      res.json({ success: true, awardedPoints: pointsToAward, newBalance: newPoints });
    } finally {
      releaseLock();
    }
  } catch (err: any) {
