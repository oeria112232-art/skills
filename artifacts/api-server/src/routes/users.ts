...
router.get("/users/:id", requireAuth, async (req: any, res): Promise<void> => {
  ...
  if (!isAdminUser && req.user?.id !== params.data.id) {
...
router.patch("/users/:id", requireAuth, async (req: any, res): Promise<void> => {
  ...
  if (!isAdminUser) {
    ...
    delete (updatePayload as any).points;
    delete (updatePayload as any).allowedPages;
    delete (updatePayload as any).pointsSignature;
    delete (updatePayload as any).streak;
  }

  const [user] = await db.update(usersTable).set(updatePayload).where(eq(usersTable.id, params.data.id)).returning();
  if (!user) { res.status(404).json({ error: "User not found" }); return; }
  await logAuditEvent({ action: "user_update", userId: req.user!.id, targetType: "user", targetId: params.data.id, details: { fields: Object.keys(updatePayload) }, req });
  res.json(serializeUser(user));
});
...
router.delete("/users/:id", requireAuth, async (req: any, res): Promise<void> => {
  ...
  if (!isAdminUser && user?.id !== params.data.id) {
    res.status(403).json({ error: "Forbidden" });
    return;
  }
...
router.post("/users/:id/avatar", requireAuth, async (req: any, res): Promise<void> => {
  ...
  const isAdminUser = req.user?.role === "admin" || req.user?.role === "super_admin";
  if (!isAdminUser && req.user?.id !== userId) {
